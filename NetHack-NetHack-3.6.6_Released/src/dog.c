...
boolean quietly;
{
    ...
    do {
        if (otmp) { /* figurine; otherwise spell */
            ...
            if ((mvitals[mndx].mvflags & G_EXTINCT)
                ...
                if (!quietly)
                    ...
                    /*KR pline("... into a pile of dust."); */
                    pline("... 그리고는 먼지 더미로 변해버렸다.");
                break; /* mtmp is null */
            }
        } else if (!rn2(3)) {
            pm = &mons[pet_type()];
            ...
            if (!pm) {
                if (!quietly)
                    /*KR There("seems to be nothing available for a familiar."); */
                    pline("시종으로 할 만한 것이 아무것도 없는 것 같다.");
                break;
            }
        }
        ...
        if (otmp && !mtmp) { /* monster was genocided or square occupied */
            if (!quietly)
                /*KR pline_The("figurine writhes and then shatters into pieces!"); */
                pline("모형이 꿈틀거리다가 산산조각났다!");
            break;
        }
    } while (!mtmp && --trycnt > 0);
    ...
    if (otmp) { /* figurine; resulting monster might not become a pet */
        ...
        if (chance > 0) {
            ...
            if (chance == 2) { /* hostile (cursed figurine) */
                if (!quietly)
                    /*KR You("get a bad feeling about this."); */
                    You("좋지 않은 예감이 든다.");
                mtmp->mpeaceful = 0;
                set_malign(mtmp);
            }
...
#else
            petname = "슬래셔"; /* The Warrior */
#endif
        if (Role_if(PM_SAMURAI))
#if 0 /*KR:T*/
            petname = "Hachi"; /* Shibuya Station */
#else
            petname = "하치"; /* Shibuya Station */
#endif
...
boolean pets_only; /* true for ascension or final escape */
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp2) {
        ...
        if (((monnear(mtmp, u.ux, u.uy) && levl_follower(mtmp))
...
#else
                    pline("%s은/는 아직 %s.", Monnam(mtmp),
                        mtmp->meating ? "먹고 있다" : "덫에 걸려 있다");
#endif
                stay_behind = TRUE;
            } else if (mon_has_amulet(mtmp)) {
                if (canseemon(mtmp))
                    /*KR pline("%s seems very disoriented for a moment.", */
                    pline("%s은/는 잠시 매우 어지러워하는 것처럼 보인다.",
                          Monnam(mtmp));
                stay_behind = TRUE;
            }
            if (stay_behind) {
            ...
            /*KR pline("%s leash goes slack.", s_suffix(Monnam(mtmp))); */
            pline("%s에 묶은 줄이 느슨해졌다.", Monnam(mtmp));
            m_unleash(mtmp, FALSE);
        }
    }
}
            ...
            if (haseyes(youmonst.data)) {
                if (haseyes(mtmp->data))
                    ...
                    pline("%s %s to look you in the %s.", Monnam(mtmp),
                          ...
                          body_part(EYE));
#else
                    pline("%s은/는 당신의 %s%s.", Monnam(mtmp),
                        body_part(EYE),
                        mtmp->mpeaceful ? "을 쳐다볼 수 없는 것 같다" : "을 피했다");
#endif
                else
                    /*KR pline("%s avoids your gaze.", Monnam(mtmp)); */
                    pline("%s은/는 당신의 시선을 회피했다.", Monnam(mtmp));
            }
        }
    } else {
    ...
    if (!mtmp->mtame) {
        if (!quietly && canspotmon(mtmp))
            ...
            pline("%s은/는 %s.", Monnam(mtmp),
                mtmp->mpeaceful ? "더이상 길들인 상태가 아니다" : "야생으로 돌아갔다");
#endif
        newsym(mtmp->mx, mtmp->my);
