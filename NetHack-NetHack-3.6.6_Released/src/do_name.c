...
static const char *const gloc_descr[NUM_GLOCS][4] = {
    ...
    { "몬스터", "몬스터", "몬스터 옆", "몬스터" },
    { "물건", "물건", "물건 옆", "물건" },
    { "문", "문", "다음/이전 문/출입구", "문들이나 출입구들" },
    { "탐색하지 않은 곳", "탐색하지 않은 곳", "미탐색 장소", "미탐색 장소" },
    { "흥미로운 것", "흥미로운 것", "흥미로운 것", "흥미로운 것" },
    { "유효한 위치", "유효한 위치", "유효한 위치", "유효한 위치" }
#endif
};
...
const char *goal;
{
...
#else
    static const char* const fastmovemode[2] = { "한 번에 8칸",
                                                 "같은 지형은 건너뛰고" };
#endif
    char sbuf[BUFSZ];
...
#else
    Sprintf(sbuf,
        "'%c', '%c', '%c', '%c'를 사용해서 %s로 이동할 수 있습니다.", /* hjkl */
        Cmd.move_W, Cmd.move_S, Cmd.move_N, Cmd.move_E, goal);
#endif
    putstr(tmpwin, 0, sbuf);
#if 0 /*KR:T*/
...
#else
    Sprintf(sbuf,
        "'H', 'J', 'K', 'L'로 %s 이동이 가능합니다.",
        fastmovemode[iflags.getloc_moveskip]);
#endif
    putstr(tmpwin, 0, sbuf);
    /*KR putstr(tmpwin, 0, "Or enter a background symbol (ex. '<')."); */
    putstr(tmpwin, 0, "배경의 기호를 입력하면 그 위치로 이동합니다. (예시: '<')");
#if 0 /*KR:T*/
    Sprintf(sbuf, "Use '%s' to move the cursor on yourself.",
           visctrl(Cmd.spkeys[NHKF_GETPOS_SELF]));
#else
    Sprintf(sbuf, "'%s'로 자신의 위치로 이동합니다.",
        visctrl(Cmd.spkeys[NHKF_GETPOS_SELF]));
#endif
    putstr(tmpwin, 0, sbuf);
    ...
    Sprintf(sbuf, "Use '%s' to change fast-move mode to %s.",
            ...
            fastmovemode[!iflags.getloc_moveskip]);
#else
    Sprintf(sbuf, "'%s'를 눌러 고속 이동 모드에서 %s 이동으로 바꿀 수 있습니다.",
        visctrl(Cmd.spkeys[NHKF_GETPOS_MOVESKIP]),
        fastmovemode[!iflags.getloc_moveskip]);
#endif
    putstr(tmpwin, 0, sbuf);
...
#else
        Sprintf(sbuf, "'%s'를 눌러 가능한 목표의 메뉴 표시를 전환합니다.",
            visctrl(Cmd.spkeys[NHKF_GETPOS_MENU]));
#endif
        putstr(tmpwin, 0, sbuf);
#if 0 /*KR:T*/
...
#else
        Sprintf(sbuf,
            "'%s'를 눌러 가능한 목표 제한의 모드를 바꿀 수 있습니다.",
            visctrl(Cmd.spkeys[NHKF_GETPOS_LIMITVIEW]));
#endif
        putstr(tmpwin, 0, sbuf);
    }
