    ...
    if (gets_hit) {
        ...
        if (uarmh) {
            if (is_metallic(uarmh)) {
                ...
                Your("%s ´ç½ÅÀ» ÁöÄÑÁÖÁö ¸øÇß´Ù.",
                     append_josa(xname(uarmh), "Àº"));
        }
#if 0 /*KR*/
        losehp(Maybe_Half_Phys(dmg), "crunched in the head by an iron ball",
               NO_KILLER_PREFIX);
#else
        losehp(Maybe_Half_Phys(dmg), "Ã¶±¸¿¡ ¸Ó¸®¸¦ ¸Â¾Æ¼­",
               KILLED_BY);
#endif
    }
}
...
xchar x, y;
{
    ...
    if (x != u.ux || y != u.uy) {
        ...
        static const char *pullmsg = "Ã¶±¸°¡ ´ç½ÅÀ» %s¿¡¼­ ²ø¾î³Â´Ù!";
        struct trap *t;
        long side;
            ...
            case TT_BEARTRAP:
                ...
                if (!u.usteed) {
                    ...
                    losehp(Maybe_Half_Phys(2),
                           ...
                           KILLED_BY);
#else 
                    Your("%s %s ½ÉÇÏ°Ô ´ÙÃÆ´Ù.",
                         (side == LEFT_SIDE) ? "¿ÞÂÊ" : "¿À¸¥ÂÊ",
                         append_josa(body_part(LEG), "°¡"));
                    losehp(Maybe_Half_Phys(2),
                           "°õµ£¿¡¼­ ºüÁ®³ª¿À´Ù ´Ù¸®¸¦ ´ÙÃÄ¼­", KILLED_BY);
#endif
                }
                break;
            }
...
litter()
{
    ...
    for (otmp = invent; otmp; otmp = nextobj) {
        ...
        if ((otmp != uball) && (rnd(capacity) <= (int) otmp->owt)) {
            if (canletgo(otmp, "")) {
#if 0 /*KR: ¿øº» (ºÒÇÊ¿äÇÑ ¿µ¾î ¼öÀÏÄ¡ ·ÎÁ÷ Á¦°Å) */
                You("drop %s and %s %s down the stairs with you.",
                    yname(otmp), (otmp->quan == 1L) ? "it" : "they",
                    otense(otmp, "fall"));
#else 
                You("%s ¶³¾î¶ß·È°í, ±×°ÍÀº ´ç½Å°ú ÇÔ²² °è´Ü ¾Æ·¡·Î ±¼·¯¶³¾îÁ³´Ù.",
                    append_josa(yname(otmp), "À»"));
#endif
                dropx(otmp);
    ...
    if (forward) {
...
#else 
            pline("Ã¶±¸¿¡ ÀÌ²ø·Á °è´Ü ¾Æ·¡·Î ±¼·¯¶³¾îÁ³´Ù!");
            losehp(Maybe_Half_Phys(rnd(6)),
                   "Ã¶±¸¿¡ ÀÌ²ø·Á °è´Ü¿¡¼­ ±¼·¯¶³¾îÁ®¼­", KILLED_BY);
#endif
            litter();
        }
    } else {
        if (rn2(2)) {
            ...
            losehp(Maybe_Half_Phys(rnd(20)), "iron ball collision",
                   KILLED_BY_AN);
#else 
            pline("Ã¶±¸°¡ ´ç½Å¿¡°Ô ¼¼°Ô ºÎµúÇû´Ù!");
            losehp(Maybe_Half_Phys(rnd(20)), "Ã¶±¸¿Í Ãæµ¹ÇØ¼­", 
                   KILLED_BY_AN);
#endif
