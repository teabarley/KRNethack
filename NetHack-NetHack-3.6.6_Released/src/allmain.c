...
boolean resuming;
{
    ...
    */
        pline("운수 좋은 날입니다! 오늘 밤은 보름달입니다.");
        change_luck(1);
    } else if (flags.moonphase == NEW_MOON) {
    /*KR
        pline("Be careful!  New moon tonight.");
    */
        pline("조심하세요! 오늘 밤은 초승달입니다.");
    }
    flags.friday13 = friday_13th();
    if (flags.friday13) {
        /*KR pline("Watch out!  Bad things can happen on Friday the 13th."); */
        pline("조심하세요! 13일의 금요일에는 나쁜 일들이 일어날 수 있습니다.");
        change_luck(-1);
    }
            ...
            do { /* hero can't move this turn loop */
                ...
                if (!monscanmove && youmonst.movement < NORMAL_SPEED) {
                    ...
                    if (u.uen < u.uenmax
                        ...
                        context.botl = TRUE;
                        if (u.uen == u.uenmax)
                            /*KR interrupt_multi("You feel full of energy."); */
                                 interrupt_multi("기력을 회복했다.");
                    }
...
int wtcap;
{
    ...
    if (reached_full)
        /*KR interrupt_multi("You are in full health."); */
        interrupt_multi("체력을 완전히 회복했다.");
}
...
boolean new_game; /* false => restoring an old game */
{
    ...
    if (!new_game && Upolyd && ugenocided()) {
        ...
        /*KR pline("You're back, but you still feel %s inside.", udeadinside()); */
        pline("당신은 돌아왔지만, 영혼은 %s인 채 그대로다.", udeadinside());
        return;
    }
    ...
    *buf = '\0';
    if (new_game || u.ualignbase[A_ORIGINAL] != u.ualignbase[A_CURRENT])
        /*KR Sprintf(eos(buf), " %s", align_str(u.ualignbase[A_ORIGINAL])); */
        Sprintf(eos(buf), "%s", align_str(u.ualignbase[A_ORIGINAL]));
    if (!urole.name.f
        && (new_game
                ? (urole.allow & ROLE_GENDMASK) == (ROLE_MALE | ROLE_FEMALE)
                : currentgend != flags.initgend))
        /*KR Sprintf(eos(buf), " %s", genders[currentgend].adj); */
        Sprintf(eos(buf), "의 %s", genders[currentgend].adj);
...
#else
    if (new_game){
        pline("%s, Nethack의 세계에! 이 게임에서 당신은 %s %s (%s)입니다.",
              Hello((struct monst *) 0), urace.adj,
              (currentgend && urole.name.f) ? urole.name.f : urole.name.m,
              buf);
    } else {
        pline("%s, Nethack의 세계에! 당신은 %s %s입니다!",
              Hello((struct monst *) 0), urace.adj,
              (currentgend && urole.name.f) ? urole.name.f : urole.name.m);
    }
#endif
