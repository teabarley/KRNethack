...
VA_DECL(const char *, line)
{
    ...
    VA_START(line);
    VA_INIT(line, const char *);
    /*KR vpline(YouMessage(tmp, "You ", line), VA_ARGS); */
    vpline(YouMessage(tmp, "당신은 ", line), VA_ARGS);
    VA_END();
}
...
VA_DECL(const char *, line)
{
    ...
    VA_START(line);
    VA_INIT(line, const char *);
    /*KR vpline(YouMessage(tmp, "Your ", line), VA_ARGS); */
    vpline(YouMessage(tmp, "당신의 ", line), VA_ARGS);
    VA_END();
}
...
VA_DECL(const char *, line)
{
    ...
    VA_INIT(line, const char *);
    if (Unaware)
        /*KR YouPrefix(tmp, "You dream that you feel ", line); */
        YouPrefix(tmp, "당신은 꿈속에서 ", line);
    else
        /*KR YouPrefix(tmp, "You feel ", line); */
        YouPrefix(tmp, "당신은 ", line);
...
VA_DECL(const char *, line)
{
    ...
    VA_START(line);
    VA_INIT(line, const char *);
    /*KR vpline(YouMessage(tmp, "You can't ", line), VA_ARGS); */
    vpline(YouMessage(tmp, "당신은 ", line), VA_ARGS);
    VA_END();
}
...
VA_DECL(const char *, line)
{
    ...
    VA_START(line);
    VA_INIT(line, const char *);
    /*KR vpline(YouMessage(tmp, "The ", line), VA_ARGS); */
...
VA_DECL(const char *, line)
{
    ...
    VA_START(line);
    VA_INIT(line, const char *);
    /*KR vpline(YouMessage(tmp, "There ", line), VA_ARGS); */
    vpline(YouMessage(tmp, "", line), VA_ARGS);
    VA_END();
}
...
VA_DECL(const char *, line)
{
    ...
    if (Underwater)
        ...
        adj = "";
    tmp = You_buf(strlen(adj) + strlen(line) + sizeof("당신은 "));

    p = (char *)strstr(line, "듣는다") ;
    if (p == NULL)
        Strcpy(tmp, "당신은 ");
    else
        Strcpy(tmp, "");
    if (p != NULL || (p = (char*)strstr(line, "듣는다")) != NULL) {
...
VA_DECL(const char *, line)
{
    ...
    VA_INIT(line, const char *);
    if (Unaware)
        /*KR YouPrefix(tmp, "You dream that you see ", line); */
        YouPrefix(tmp, "당신은 꿈 속에서 ", line);
#if 0 /*KR*/ /*여기는 호출원으로 처리할까? -파파고 번역- */
    else if (Blind) /* caller should have caught this... */
        YouPrefix(tmp, "You sense ", line);
#endif
    else
        /*KR YouPrefix(tmp, "You see ", line); */
...
VA_DECL(const char *, line)
{
    ...
    VA_START(line);
    VA_INIT(line, const char *);
    /*JNethack에서 이 부분은 ㄴㅇㄱ 기호를 맛깔나게 써줍니다. 전 그냥 함.*/
...
VA_DECL(const char *, s)
{
    ...
    /* reuse pbuf[] */
    /*KR Strcpy(pbuf, "Program in disorder!"); */
    Strcpy(pbuf, "프로그램에 오류 발생!");
    if (program_state.something_worth_saving)
        /*KR Strcat(pbuf, "  (Saving and reloading may fix this problem.)"); */
        Strcat(pbuf, "  (저장한 다음 다시 불러오면 이 문제가 해결될 수도 있습니다.)");
    pline("%s", VA_PASS1(pbuf));
