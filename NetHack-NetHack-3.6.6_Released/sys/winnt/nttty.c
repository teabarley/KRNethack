...
static void back_buffer_flip()
{
    ...
    /* 1패스: 글자가 깨지지 않도록 속성(배경색/글자색)을 화면 전체에 먼저
     * 바릅니다. */
    back = console.back_buffer;
    front = console.front_buffer;
    for (pos.Y = 0; pos.Y < console.height; pos.Y++) {
        for (pos.X = 0; pos.X < console.width; pos.X++) {
            if (back->attribute != front->attribute) {
                WriteConsoleOutputAttribute(console.hConOut, &back->attribute,
    ...
    /* 2패스: 색상이 다 발라진 안전한 도화지 위에 글자를 얹습니다. */
    back = console.back_buffer;
    front = console.front_buffer;
    for (pos.Y = 0; pos.Y < console.height; pos.Y++) {
        for (pos.X = 0; pos.X < console.width; pos.X++) {
            if (back->character != front->character) {
                if (back->character != L'\0') { /* 더미 문자는 그리지 않음 */
                    if (console.has_unicode) {
                        WriteConsoleOutputCharacterW(console.hConOut,
...
int ch;
{
    ...
    set_console_cursor(ttyDisplay->curx, ttyDisplay->cury);
#else
    extern int cp949_idx;
 /* KR: 한글 바이트 조립 중(Trail byte)일 때는 커서를 동기화하지 않음 */
    if (cp949_idx == 0) {
        set_console_cursor(ttyDisplay->curx, ttyDisplay->cury);
    }
#endif
...
void xputs(s) const char *s;
{
    int k;
    int slen = (int) strlen(s);

#if 0 /*KR 한글 조립기 찌꺼기 완벽 차단 */
    if (ttyDisplay)
        set_console_cursor(ttyDisplay->curx, ttyDisplay->cury);
...
#else
    extern int cp949_idx; /* KR: 한글 조립기 상태 변수 */

    if (ttyDisplay)
        set_console_cursor(ttyDisplay->curx, ttyDisplay->cury);

    cp949_idx = 0; /* KR: 새 문자열 출력 전 찌꺼기 무조건 초기화! */
...
int in_ch;
{
    ...
    /* KR: 넷핵 코어(topl.c)의 linestart 버그 방어.
       맵 창(NHW_MAP)이 아닌데 g_putch가 호출되었다면 한글 첫 바이트가 잘못
       넘어온 것이므로 안전한 텍스트 전용 xputc_core로 라우팅합니다. */
    if (ttyDisplay->lastwin < 0 || wins[ttyDisplay->lastwin]->type != NHW_MAP
        || cp949_idx != 0) {
        xputc_core(ch);
        return;
    }
